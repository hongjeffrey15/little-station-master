...
gen(){ # gen <name> <espeak-text> [speed] [pitch]
  espeak-ng -v mb-en1 -s "${3:-135}" -p "${4:-60}" "$2" -w "$OUT/$1.wav" 2>/dev/null
...
for w in cat dog sun pig bus hat pin bed tin nap man dad tap snake apple tiger insect nose mouse; do gen "w_$w" "$w" 130 58; done
